                  ...
                  <tr key={ro.id} className="hover:bg-white/5 transition-smooth group cursor-pointer" onClick={() => navigate(`/repair-orders/${ro.id}`)}>
                    ...
                    <td className="p-4" onClick={(e) => e.stopPropagation()}>
                      <select
                        ...
                        className={`text-[10px] font-black uppercase px-2 py-1 rounded bg-surface border border-border outline-none cursor-pointer hover:border-primary transition-smooth ${ro.status === 'In Progress' ? 'text-info' : ro.status === 'Completed' ? 'text-success' : 'text-warning'
                          }`}
                      >
